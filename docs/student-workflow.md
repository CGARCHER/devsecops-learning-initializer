...
Con la conexión indicada en la guía, un estado `APPROVED` permite desplegar automáticamente al terminar el análisis de un push a `main`. El despliegue comprueba la salida `allowed` y utiliza el commit de la salida `sha`. Para `BLOCKED` o `REVIEW_REQUIRED`, revisa el informe y marca «Acepto los hallazgos del análisis» al lanzar manualmente el despliegue. Espera a que terminen todos los analizadores, incluido `container`, y el informe final. No necesitas copiar el SHA ni escribir un comentario. Los errores técnicos, informes ausentes o análisis incompletos detienen el despliegue. Aceptar el riesgo no elimina los hallazgos.
